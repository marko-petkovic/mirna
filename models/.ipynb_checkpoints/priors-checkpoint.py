...
class combined_conv_prior(nn.Module):
    
    def __init__(self, z_dim, y_nodes, batchnorm=True, nonlin=nn.LeakyReLU()):
        
        super(combined_conv_prior, self).__init__()
        ...
        fc = [nn.Linear(2, y_nodes),
              nn.BatchNorm1d(y_nodes) if batchnorm else None,
              nonlin,
              nn.Linear(y_nodes, y_nodes),
              nn.BatchNorm1d(y_nodes) if batchnorm else None,
              nonlin]
        fc = [i for i in fc if i is not None]
        self.fc = nn.Sequential(*fc)
        
        self.mu = nn.Sequential(nn.Linear(y_nodes+64, z_dim))
        self.sigma = nn.Sequential(nn.Linear(y_nodes+64, z_dim,), nn.Softplus())
